	...
	<form action="" method="get"  class="form-search">
		...
		<input type="hidden" name="how" value="<?echo $arResult["REQUEST"]["HOW"]=="d"? "d": "r"?>" />
				<div class="input-append">
					...
					<?else:?>
						<input type="text" name="q" class="span<?=$GLOBALS["main_column_width"]-2?> search-query" placeholder="<?echo GetMessage("CT_BSP_SEARCH_PLACEHOLDER")?>" value="<?=$arResult["REQUEST"]["QUERY"]?>" />
					<?endif;?>
					<button class="btn" type="submit" /><i class="icon-search"></i> <?echo GetMessage("CT_BSP_GO")?></button>
				</div>
		<noindex>
		<div class="search-advanced">
			...
			<?if($arParams["SHOW_WHERE"] || $arParams["SHOW_WHEN"]):?>
				...
				<div id="search_params" class="search-filter" style="display:<?echo $arResult["REQUEST"]["FROM"] || $arResult["REQUEST"]["TO"] || $arResult["REQUEST"]["WHERE"]? 'block': 'none'?>">
					<h6><?echo GetMessage('CT_BSP_ADVANCED_SEARCH')?></h6>
					<table class="search-filter"><tbody>
						<?if($arParams["SHOW_WHERE"]):?>
						<tr>
